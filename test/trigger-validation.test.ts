...
import {
  ...
  mockGiteaPullRequestReviewCommentContext,
  mockUndefinedCommentContext,
} from "./mockContext";
import type {
  IssueCommentEvent,
  IssuesAssignedEvent,
...
describe("comment trigger", () => {
  ...
  it("should return true for Gitea pull_request_review_comment with trigger in review.body", () => {
    // Set GITEA_API_URL to simulate Gitea environment
    process.env.GITEA_API_URL = "https://gitea.example.com";
    const context = mockGiteaPullRequestReviewCommentContext;
    expect(checkContainsTrigger(context)).toBe(true);
    delete process.env.GITEA_API_URL; // Clean up
  });

  it("should return false for pull_request_review_comment with undefined comment body", () => {
    const context = mockUndefinedCommentContext;
    expect(checkContainsTrigger(context)).toBe(false);
  });

  it("should not crash when payload.comment is undefined", () => {
    const context = mockUndefinedCommentContext;
    expect(() => checkContainsTrigger(context)).not.toThrow();
  });
});
